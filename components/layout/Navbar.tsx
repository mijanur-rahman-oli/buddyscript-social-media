...
function NotificationDropdown({ open }: { open: boolean }) {
  const notifications = [
    { id: "1", user: "Steve Jobs",  text: "posted a link in your timeline.",               time: "42 minutes ago", avatar: "/assets/images/friend-req.png"  },
    { id: "2", user: "Admin",       text: "changed the name of the group Freelancer USA.", time: "42 minutes ago", avatar: "/assets/images/profile-1.png"   },
    { id: "3", user: "Steve Jobs",  text: "posted a link in your timeline.",               time: "1 hour ago",     avatar: "/assets/images/friend-req.png"  },
    { id: "4", user: "Admin",       text: "changed the name of the group Freelancer USA.", time: "2 hours ago",    avatar: "/assets/images/profile-1.png"   },
  ];

  return (
    <div className={`_notification_dropdown${open ? " show" : ""}`}>
        ...
        <div className="_notification_box_right">
          ...
          <div className="_notifications_drop_right">
            <ul className="_notification_list">
              ...
              <li className="_notification_item"><span className="_notification_link">Notification settings</span></li>
            ...
            <div key={n.id} className="_notification_box">
              ...
              <div className="_notification_txt">
                <p className="_notification_para">
                  <span className="_notify_txt_link">{n.user}</span> {n.text}
                </p>
...
// ─── Profile Dropdown ─────────────────────────────────────────────────────────
function ProfileDropdown({ open, user }: { open: boolean; user: NavbarProps["user"] }) {
  return (
    <div className={`_nav_profile_dropdown${open ? " show" : ""}`}>
      <div className="_nav_profile_dropdown_info">
        {user.image ? (
          <Image src={user.image} alt={user.name ?? "Profile"} width={40} height={40} className="_nav_drop_img" />
        ) : (
          <div className="_nav_drop_img" style={{
            background: "#377DFF", display: "flex", alignItems: "center",
            justifyContent: "center", color: "#fff", fontWeight: 700,
            fontSize: 16, borderRadius: "50%", width: 40, height: 40, flexShrink: 0,
          }}>
            {user.name?.[0] ?? "U"}
          </div>
        )}
      ...
      <hr style={{ margin: "12px 0", borderColor: "var(--bs-border)" }} />
...
export function Navbar({ user }: NavbarProps) {
  ...
  return (
    <nav className="_header_nav">
      <div className="_nav_inner">

        {/* ── Logo ─────────────────────────────────────── */}
        <Link href="/feed" className="_nav_logo_wrap">
          <img src="/assets/images/logo.svg" alt="BuddyScript" className="_nav_logo" />
        </Link>

        {/* ── Search — grows to fill space ─────────────── */}
        <div className="_nav_search">
          <form className="_header_form_grp" role="search">
            <svg className="_header_form_svg" xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 17 17">
              <circle cx="7" cy="7" r="6" stroke="#666"/>
              <path stroke="#666" strokeLinecap="round" d="M16 16l-3-3"/>
            </svg>
            <input className="_inpt1" type="search" placeholder="input search text" aria-label="Search" />
          </form>
        </div>
        ...
        <ul className="_nav_icons">
          ...
          <li className="_header_nav_item">
            <Link
              href="/feed"
              className={`_header_nav_link${isActive("/feed") ? " _header_nav_link_active" : ""}`}
              aria-label="Home"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="21" fill="none" viewBox="0 0 18 21">
                <path stroke="currentColor" strokeWidth="1.5" strokeOpacity=".6" d="M1 9.924c0-1.552 0-2.328.314-3.01.313-.682.902-1.187 2.08-2.196l1.143-.98C6.667 1.913 7.732 1 9 1c1.268 0 2.333.913 4.463 2.738l1.142.98c1.179 1.01 1.768 1.514 2.081 2.196.314.682.314 1.458.314 3.01v4.846c0 2.155 0 3.233-.67 3.902-.669.67-1.746.67-3.901.67H5.57c-2.155 0-3.232 0-3.902-.67C1 18.002 1 16.925 1 14.77V9.924z"/>
                <path stroke="currentColor" strokeOpacity=".6" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M11.857 19.341v-5.857a1 1 0 00-1-1H7.143a1 1 0 00-1 1v5.857"/>
              </svg>
            </Link>
          </li>
          ...
          <li className="_header_nav_item">
            <Link
              href="/friend-request"
              className={`_header_nav_link${isActive("/friend-request") ? " _header_nav_link_active" : ""}`}
          ...
          {/* Notifications */}
          <li ref={notifyRef} className="_header_nav_item" style={{ position: "relative" }}>
            <button
              className={`_header_nav_link${notifyOpen ? " _header_nav_link_active" : ""}`}
              onClick={() => { setNotifyOpen(p => !p); setProfileOpen(false); }}
          ...
          <li className="_header_nav_item">
            <Link
              href="/chat"
              className={`_header_nav_link${isActive("/chat") ? " _header_nav_link_active" : ""}`}
              aria-label="Messages"
            >
        ...
        {/* ── Profile ──────────────────────────────────── */}
        <div ref={profileRef} className="_nav_profile_wrap">
          <button
            className="_nav_profile_btn"
            onClick={() => { setProfileOpen(p => !p); setNotifyOpen(false); }}
          ...
          >
            ...
            {user.image ? (
              ...
              <div className="_nav_avatar_fallback">
                {user.name?.[0] ?? "U"}
              </div>
            )}
            {/* Name */}
            <span className="_nav_profile_name">{user.name}</span>
            {/* Chevron */}
            <svg
          ...
          <ProfileDropdown open={profileOpen} user={user} />
        </div>
